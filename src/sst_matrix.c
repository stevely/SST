...
GLfloat zfar, GLfloat *mat ) {
    ...
    mat[11] = (2 * znear * zfar) / (znear - zfar);
    /**/
    mat[12] = 0.0f;
    mat[13] = 0.0f;
    mat[14] = -1.0f;
...
GLfloat * sstMatMult3( GLfloat *m1, GLfloat *m2 ) {
    GLfloat *mat;
    mat = sstGenMatrix3x3();
    mat[0] = (m1[0] * m2[0]) + (m1[1] * m2[3]) + (m1[2] * m2[6]);
    mat[1] = (m1[0] * m2[1]) + (m1[1] * m2[4]) + (m1[2] * m2[7]);
    mat[2] = (m1[0] * m2[2]) + (m1[1] * m2[5]) + (m1[2] * m2[8]);
    /**/
    mat[3] = (m1[3] * m2[0]) + (m1[4] * m2[3]) + (m1[5] * m2[6]);
    mat[4] = (m1[3] * m2[1]) + (m1[4] * m2[4]) + (m1[5] * m2[7]);
    mat[5] = (m1[3] * m2[2]) + (m1[4] * m2[5]) + (m1[5] * m2[8]);
    /**/
    mat[6] = (m1[6] * m2[0]) + (m1[7] * m2[3]) + (m1[8] * m2[6]);
    mat[7] = (m1[6] * m2[1]) + (m1[7] * m2[4]) + (m1[8] * m2[7]);
    mat[8] = (m1[6] * m2[2]) + (m1[7] * m2[5]) + (m1[8] * m2[8]);
    return mat;
}
